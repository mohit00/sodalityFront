...
@Component({
  selector: 'app-monthly-data-date-wise-csv',
  templateUrl: './monthly-data-date-wise-csv.component.html',
  styleUrls: ['./monthly-data-date-wise-csv.component.scss']
})
export class MonthlyDataDateWiseCsvComponent implements OnInit {
  Formgroup: FormGroup;
...
tokenId:any;
  ...
  constructor(private pipe:DatePipe,private fb :FormBuilder,private ReportService:ReportServiceService,private FacilityService:FacilityServiceService) {
    ...
    this.Formgroup = this.fb.group({
      ...
      to_date: ['', [
       ]], report: ['monthly_data_flat_wise_csv'],
       format:['csv'],
       from_flat:['', [
        Validators.required
      ]],
